...
// ============================
// VIDEO "LA NASCITA DEL PROGETTO" (play overlay)
// ============================
(function initStoryVideo(){
  ...
  const heroBtn = document.getElementById('btnVideo');

  if (!video || !playBtn || !card) return;
  ...
  function playWithSound(){
    try{
      ...
      if (p && typeof p.then === 'function'){
        p.catch(()=>{/* autoplay policy or missing file */});
      }
  ...
  if (heroBtn){
    heroBtn.addEventListener('click', ()=>{
      // Avvio video solo dopo click esplicito (policy browser). Qui lasciamo la scelta all’utente.
      setTimeout(()=>{ playBtn.focus({preventScroll:true}); }, 450);
    });
  }
})();
